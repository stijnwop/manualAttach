...
source(ManualAttaching.modDir .. 'scripts/events/ManualAttachingPTOEvent.lua')
source(ManualAttaching.modDir .. 'scripts/events/ManualAttachingDynamicHosesEvent.lua')
-- source(ManualAttaching.modDir .. 'scripts/events/ManualAttachingSoundEvent.lua')
...
function ManualAttaching:loadMap(filename)
	...
	self.detachingNotAllowedTime = (math.sin(2000 / 100) + 1) * 0.5
	self.currentSoundPlayer = nil
	self.playerDistance = math.huge
	self.playerMinDistance = 9
	self.cosAngleLimit = math.cos(math.rad(70)) -- int 70°
	...
	if self:isPlayer() then				
		...
		if ir.vehicle ~= nil then
			...
			if ir.attachedImplement ~= nil then
				local implement = ir.vehicle:getImplementByObject(ir.attachedImplement)

				if implement ~= nil then
					local jointDesc = ir.vehicle.attacherJoints[implement.jointDescIndex]
					...
					if InputBinding.isPressed(InputBinding.IMPLEMENT_EXTRA4) then
						if player.manualAttachingTimer.time < 200 then
		...
		if ir.vehicle ~= nil then
			if ir.attachableInMountRange ~= nil then
				...
				if table.getn(ir.vehicle.attachedImplements) > 0 then
...
---
--
function ManualAttaching:resetManualTable()
	local ir = self.inRangeManual
...
-- @param vehicle
--
function ManualAttaching:setInRangeManual(vehicle)
	...
	if vehicle.attachedImplements ~= nil then
		for i, implement in pairs(vehicle.attachedImplements) do
			...
			if distance < self.playerMinDistance and distance < self.playerDistance then
				ir.vehicle = vehicle
						...
						if attachable.attacherVehicle == nil then
							for i, inputAttacherJoint in pairs(attachable.inputAttacherJoints) do
								if attachable:getIsInputAttacherActive(inputAttacherJoint) and inputAttacherJoint.jointType == attacherJoint.jointType then
									...
									if distanceJoints < nearestDisSequence then
										if (math.abs(inputJointTrans[2] - jointTrans[2])) < 1.3 then
											local cosAngle = ManualAttaching:calculateCosAngle(inputAttacherJoint.node, attacherJoint.jointTransform)

											if cosAngle > self.cosAngleLimit or ManualAttaching:getDoesNotNeedCosAngleValidation(attacherJoint) then
												ir.vehicle = vehicle
												-- ir.attachedImplement = nil
												...
												self.playerDistance = distance
												nearestDisSequence = distanceJoints
											end
										end
									end
								end
					...
					if vehicle.attachedImplements ~= nil then
						...
						if implementIndex ~= nil then
							...
							if object ~= nil then
								...
								-- for a, attachedImplementsAttacherJoint in pairs(attachedImplement.object.attacherJoints) do
									-- if inputAttacherJoint.jointType == attachedImplementsAttacherJoint.jointType then
										-- local attachedJointTrans = {getWorldTranslation(attachedImplementsAttacherJoint.jointTransform)}
										-- local distance = Utils.vector2LengthSq(inputJointTrans[1] - attachedJointTrans[1], inputJointTrans[3] - attachedJointTrans[3])

										-- if distance < nearestDisSequence then
											-- if (math.abs(inputJointTrans[2] - attachedJointTrans[2])) < 1.3 then
												-- local cosAngle = ManualAttaching:calculateCosAngle(inputAttacherJoint.node, attachedImplementsAttacherJoint.jointTransform)

												-- if cosAngle > self.cosAngleLimit then
													-- if not self:isChildManual(attachedImplement.object, inputAttacherJoint) then																																		
														-- ir.vehicle = attachable
														-- ir.attachableInMountRange = attachedImplement.object
														-- ir.attachableInMountRangeInputJoint = inputAttacherJoint
														-- ir.attachableInMountRangeInputJointIndex = i
														-- ir.attachableInMountRangeJoint = attachedImplementsAttacherJoint
														-- ir.attachableInMountRangeJointIndex = a
...
function ManualAttaching:disableDetachRecursively(vehicle)
	local numImplements = table.getn(vehicle.attachedImplements)
...
function ManualAttaching:scopeAllowsDetaching(object, jointDesc, showWarning)
	...
	if warning ~= nil and showWarning then
		...
		g_currentMission:enableHudIcon('detachingNotAllowed', 6, self.detachingNotAllowedTime)
	end
...
function ManualAttaching:handleAttachDetach()
	...
	elseif ir.attachedImplement ~= nil then
		self:detachImplement(ir.vehicle, ir.attachedImplement, ir.attachedImplementIndex)
	end
end
	...
	if vehicle ~= nil and object ~= nil then
		...
		if jointDesc ~= nil and inputJointDesc ~= nil then
			if ManualAttaching:isManual(object, inputJointDesc) or force then
                ...
                vehicle.manualAttachingForcedActiveTime = g_currentMission.time + 600 
		...
		if implement ~= nil then			
			if jointDesc.dynamicHoseIndice ~= nil and implementJoint.dynamicHoseIndice ~= nil then
				if implementJoint.dynamicHoseIsAttached then		
					if ir.attachedImplement.dynamicHoseIsManual then
						if not ((ir.attachedImplement.getIsTurnedOn ~= nil and ir.attachedImplement:getIsTurnedOn()) or (ir.vehicle.getIsTurnedOn ~= nil and ir.vehicle:getIsTurnedOn())) then
							self:detachDynamicHoses(ir.attachedImplement, ir.vehicle, implement.jointDescIndex)
							self:playSound(ir.vehicle, implementJoint, self.currentSoundPlayer)
						else
					...
					if ir.attachedImplement.dynamicHoseIsManual then
						self:attachDynamicHoses(ir.attachedImplement, ir.vehicle, implement.jointDescIndex)
						self:playSound(ir.vehicle, implementJoint, self.currentSoundPlayer)
					end
				end
			end
...
function ManualAttaching:handlePowerTakeOff(jointDesc)
	...
	if g_currentMission.player.manualAttachingTimer.time > 0 then
		if g_currentMission.player.manualAttachingTimer.time < 200 then
			if ir.vehicle ~= nil and ir.attachedImplement ~= nil then
				if ManualAttaching:hasPowerTakeOff(ir.attachedImplement) then
					if self:isPtoManual(jointDesc) then
						if ManualAttaching:getCanDetachPowerTakeOff(jointDesc, 'pto') or ManualAttaching:getCanDetachPowerTakeOff(jointDesc, 'pto2') or ManualAttaching:getCanDetachPowerTakeOff(jointDesc, 'movingPto') then
							if not ((ir.attachedImplement.getIsTurnedOn ~= nil and ir.attachedImplement:getIsTurnedOn()) or (ir.vehicle.getIsTurnedOn ~= nil and ir.vehicle:getIsTurnedOn())) then
								self:detachPowerTakeOff(ir.vehicle, ir.attachedImplement)
								self:playSound(ir.vehicle, jointDesc, self.currentSoundPlayer)
							else
								ManualAttaching:showWarning(ManualAttaching.message.turnOffWarning, ir.attachedImplement)
								g_currentMission:enableHudIcon('detachingNotAllowed', 6, self.detachingNotAllowedTime)
							end
						else
							self:attachPowerTakeOff(ir.vehicle, ir.attachedImplement)
							self:playSound(ir.vehicle, jointDesc, self.currentSoundPlayer)
						end
					end
				end
...
function ManualAttaching:isManual(vehicle, jointDesc, type)
	if not (
		jointDesc.jointType == AttacherJoints.jointTypeNameToInt['skidSteer'] or 
		...
		jointDesc.jointType == AttacherJoints.jointTypeNameToInt['semitrailerHook']
	) then
...
function ManualAttaching:getIsJointMoveDownAllowed(object, jointDesc)	
	if object ~= nil then
		...
		if object.mower ~= nil and table.getn(object.foldingParts) > 1 then
			return false
		end
	end
